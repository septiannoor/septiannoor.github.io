...
<script type="text/javascript">
	...
	var colourChanger = new function() {
		...
		this.init = function() {
			...
			$("#p2").html(x_getLangInfo(x_languageData.find("colourChanger").find("adviceTxt")[0], "label", "Need more advice? Visit the <a target='_blank' href='https://xot.xerte.org.uk/play.php?template_id=151'>Xerte Online Toolkits guide to accessibility</a>."));
			...
			styles.push(['.filter_inv #x_mainHolder', 'background:black; filter:none;']);
			...
			styles.push(['.filter_boy #x_headerBlock h1', 'color:#ffff99; background:black; filter:none;']);
			styles.push(['.filter_boy #x_headerBlock h2', 'color:#ffff99; background:black; filter:none;']);
			styles.push(['.filter_boy #x_mainHolder', 'background:#ffff99; filter:none;']);
			...
			for (var i=0; i<this.filterMap.length; i++) {
				select.push('<div class="optionGroup"><input type="radio" id="option' + i + '" name="colourChangerRadios" value="' + i + '"');
				if (i == this.selectedIndex) select.push(' checked="checked"');
				select.push('><label for="option' + i + '"><p>' + x_getLangInfo(x_languageData.find("colourChanger").find(this.filterMap[i].name)[0], "label", this.filterMap[i].default) + '</p></label></div>');
			}
			...
			$("input[name='colourChangerRadios']").click(function(){
				switch(this.value){
					case "0":
					    colourChanger.switchTheme(x_params.theme);
					...
					case "1":
					 ...
					 break;
					case "2":
					    colourChanger.switchTheme("highcontrast");
                        colourChanger.applyFilter($(this).attr('value'));
					 break;
					case "3":
                        colourChanger.switchTheme("blackonyellow");
                        colourChanger.applyFilter($(this).attr('value'));
				 }
		...
		this.switchTheme = function(theme){
			// Do NOT touch theme_responisive_css
            currentThemeURL = x_themePath + theme + '/' + theme;
            $("#theme_css").attr("href", currentThemeURL + ".css");
            $("#theme_js").attr("href", currentThemeURL + ".js");
